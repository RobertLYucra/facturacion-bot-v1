...
if len(sys.argv) > 1:
    ...
    BASE_PATH = "inboxFacturas/RV_ Facturación Perú 23.10.2024"
        ...
        for idx, row in enumerate(df_analisis.head(num_filas_analizar).itertuples(index=False)):
            ...
            if valor and valor.lower() != 'nan':
                if patron_comprobante.match(valor):
                    ...
                    log(f"    ✓ Coincide con patrón de comprobante")
                else:
                    log(f"    ✗ No coincide con patrón de comprobante")

                if patron_proyecto.match(valor):
                    valores_proyecto += 1
                    log(f"    ✓ Coincide con patrón de proyecto (6 caracteres alfanuméricos)")
                else:
...
def buscar_proyecto_tabla_CSV(df_csv, codigo_busqueda, columna_proyecto=3):
    codigo_busqueda = normaliza_comprobante(codigo_busqueda)
    columna_texto = df_csv.iloc[:, 0].astype(str).apply(normaliza_comprobante)
    mascara = columna_texto == codigo_busqueda
    resultado = df_csv[mascara]
    if not resultado.empty:
        print(f"Proyecto encontrado para '{codigo_busqueda}': {resultado.iloc[0, columna_proyecto]}")
        return resultado.iloc[0, columna_proyecto]
    print(f"No se encontró comprobante '{codigo_busqueda}' en el CSV.")
    return None
...
def extract_invoice_data(xml_content):
    ...
    try:
        ...
        if description is not None and description.text:
            ...
            nr_match = re.search(r'(?:NR[:#._\s]*|N[°º]?\s*NR[:#._\s]*|Nro\.?\s*NR[:#._\s]*|Numero\s*(?:de)?\s*Recepcion\s*(?:N[°º]?)?\s*)(\d{1,7}(?:-\d{1,4})?)', texto_limpio)
            if nr_match:
                nr = nr_match.group(1)
            else:
                # Buscar alternativas como texto antes o después de una barra
                nr_match = re.search(r'[/|]\s*NR\s*[:|=]?\s*(\d{1,7}(?:-\d{1,4})?)', texto_limpio)
                if nr_match:
                    nr = nr_match.group(1)
        ...
        if not nr:
            ...
            if order_reference is not None and order_reference.text:
                ...
                nr_match = re.search(r'(?:NR[:\s]\s*|N[°º]?\s*NR[:\s]\s*|Nro\.?\s*NR[:\s]\s*|Numero\s*(?:de)?\s*Recepcion\s*(?:N[°º]?)?\s*)(\d{1,7}(?:-\d{1,4})?)', texto_limpio)
                if nr_match:
                    nr = nr_match.group(1)
                else:
                    nr_match = re.search(r'[/|]\s*NR\s*[:|=]?\s*(\d{1,7}(?:-\d{1,4})?)', texto_limpio)
...
def process_xml_files(asunto_correo):
    """
    ...
    try:
        ...
        if not xml_files:
            error_msg = f"No se encontraron archivos XML en {DIRECTORY_FILE_XML}"
            print(error_msg)
            registrar_log_detallado(asunto_correo, "3.Lectura XML", "Error", error_msg)
            return None, None
        ...
        error_msg = f"Error al procesar XML: {str(e)}"
...
if __name__ == "__main__":
    ...
    new_excel, existing_excel = process_xml_files(asunto_correo)
